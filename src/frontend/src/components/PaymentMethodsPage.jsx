...
export default function PaymentMethodsPage({ token }) {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/PaymentMethods?page=${currentPage}&pageSize=${pageSize}`, {
      ...
      setErrorMessage('Nao foi possivel carregar as formas de pagamento.')
      setItems([])
      setTotalItems(0)
    } finally {
      setIsLoading(false)
  ...
  async function handleOpenEdit(uid) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/PaymentMethods/${uid}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      setFormData({
        ...
        maxInstallments: String(payload?.maxInstallments ?? '1'),
        displayOrder: String(payload?.displayOrder ?? '1'),
      })
      setEditingUid(uid)
      setIsFormOpen(true)
    } catch {
      setErrorMessage('Nao foi possivel carregar a forma de pagamento para edicao.')
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    const payload = {
      ...
      maxInstallments: Number(formData.maxInstallments || 0),
    ...
    try {
      const response = await fetch(
        isEditing ? `${API_BASE_URL}/PaymentMethods/${editingUid}` : `${API_BASE_URL}/PaymentMethods`,
        {
      ...
      if (!response.ok) {
        ...
        const fallback = isEditing
          ...
          : 'Nao foi possivel criar a forma de pagamento.'
      ...
      setSaveErrorMessage(isEditing ? 'Nao foi possivel atualizar a forma de pagamento.' : 'Nao foi possivel criar a forma de pagamento.')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/PaymentMethods/${pendingDeleteItem.uid}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      setErrorMessage('Nao foi possivel excluir a forma de pagamento.')
    } finally {
      setDeletingUid(null)
    }
  }
  ...
  return (
    <section className="rounded-2xl bg-white p-6 shadow-md transition hover:shadow-xl dark:bg-gray-900 dark:shadow-black/30">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Formas de Pagamento</h2>
        <button
          type="button"
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.uid} className="border-b border-gray-50 dark:border-gray-800">
                    ...
                    <td className="py-2 pr-3 text-gray-700 dark:text-gray-300">{Number(item.discountPercentage ?? 0)}%</td>
                    <td className="py-2 pr-3 text-gray-700 dark:text-gray-300">{item.maxInstallments}</td>
                    <td className="py-2 pr-3 text-gray-700 dark:text-gray-300">{item.displayOrder}</td>
          ...
          <div className="mx-auto w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900">
            <header className="mb-5">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">
                {editingUid ? 'Editar' : 'Adicionar'} forma de pagamento
              </h3>
            </header>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <label className="grid gap-1 text-sm text-gray-700 dark:text-gray-300">
                  ...
                  <input
                    ...
                    onChange={(event) => handleInputChange('discountPercentage', event.target.value)}
                    required
                    className="rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                  />
                </label>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="grid gap-1 text-sm text-gray-700 dark:text-gray-300">
                  ...
                  <input
                    ...
                    onChange={(event) => handleInputChange('maxInstallments', event.target.value)}
                    required
                    className="rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
                  />
                </label>
